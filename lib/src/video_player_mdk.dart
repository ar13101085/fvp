...
class MdkVideoPlayerPlatform extends VideoPlayerPlatform {
  ...
  // Per-Player audio backend list, applied in [create] before the
  // stream opens. Lets callers opt out of mdk's default AAudio path
  // on Android — that path has a teardown race where the AAudio
  // callback thread fires after the AudioStreamLegacy C++ object's
  // vtable has been destroyed, producing a "Pure virtual function
 ...
 */
  static void registerVideoPlayerPlatformsWith({dynamic options}) {
    ...
    if (options is Map<String, dynamic>) {
      ...
      // Optional ordered list of mdk audio backend names — see
      // [_audioBackends] above. Accepted via the same [registerWith]
      // call so callers don't need to reach into individual Players.
      final ab = options['audioBackends'];
      if (ab is List) {
        _audioBackends = ab.cast<String>();
      }
    }
...
// registerWith() can be invoked by dart_plugin_registrant.dart before main. when debugging, won't enter main if posting message from native to dart(new native log message) before main?
  ...
  Future<int?> create(DataSource dataSource) async {
    ...
    if (_audioBackends != null && _audioBackends!.isNotEmpty) {
      // mdk picks the first backend in the list it can open; the
      // remainder are fallbacks. Apply BEFORE setting media so the
      // first audio decode goes through the chosen backend.
